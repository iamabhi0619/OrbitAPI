...
const updateServiceAvailed = require("../../utility/UpdateService");
const { userDataSanitization } = require("../../utility/sanitization");
...
exports.register = async (req, res, next) => {
  try {
    ...
    await newUser.save();
    const token = jwt.sign({ userId: newUser._id }, process.env.SECRET, { expiresIn: "10m" });
...
exports.login = async (req, res, next) => {
  try {
    ...
    const token = jwt.sign({ role: user.role, _id: user._id }, process.env.SECRET, { expiresIn: "5d" });
...
exports.forgetPassword = async (req, res, next) => {
  try {
    ...
    const resetToken = jwt.sign({ email }, process.env.SECRET, { expiresIn: "10m" });
    resetTokens.set(email, { token: resetToken, expiresAt: Date.now() + 10 * 60 * 1000 });
    const resetLink = `${process.env.CLIENT_URL}/auth/changePassword?token=${resetToken}&email=${email}`;
    await sendEmail({
      to: user.email,
